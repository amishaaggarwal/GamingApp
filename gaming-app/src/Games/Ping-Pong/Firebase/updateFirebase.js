      ...
      case "GameID":
        switch (keys) {
          ...
          case "total_games_played_by":
            ...
            break;
          case "total_games":
            getExistingPlayerData("GameID", `ping-pong/total_games`).then((res) => {
              let newval = res ? parseInt(res) : 0;
              update(ref(db, `${endpoint}/${newId}`), {
      ...
      case "UserList":
        ...
        switch (keys) {
          ...
          case "total_games":
            {
              let newval;
              getExistingPlayerData("UserList", `${newId}/total_games`).then(
                (res) => {
                  newval = res ? parseInt(res) : 0;
                  update(ref(db, `${endpoint}/${newId}`), {
                    total_games: newval + 1,
          ...
          case "scoreCredit":
            {
              let newval = 0;
              getExistingPlayerData("UserList", `${newId}/totalScore`).then(
                (res) => {
                  newval = res ? parseInt(res) : 0;
                  update(ref(db, `${endpoint}/${newId}`), {
